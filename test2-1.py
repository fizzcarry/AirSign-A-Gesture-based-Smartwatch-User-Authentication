...
intermediate_results_path=os.path.join(intermediate_results_base_path,"max_dist_dict.json")
# with open(intermediate_results_path, 'w') as file:
#     json.dump(max_dist_dict, file)
with open(intermediate_results_path, 'r') as file:
...
for i,json_file in enumerate(json_files):
    ...
    if pre1_flag not in sample_one_pre_dict:
        sample_one_pre_dict[pre1_flag] = DTW_classification1(one_sample_dict, max_dist_dict, np.array(data_dict["series1"]).T, feature_type=feature_type)
    if pre2_flag not in sample_one_pre_dict:
        sample_one_pre_dict[pre2_flag] = DTW_classification1(one_sample_dict, max_dist_dict, np.array(data_dict["series2"]).T, feature_type=feature_type)
